...
def _get_subset_idx(dataset, current_classes_idxs):
    ...
    no_cc_idx = random.choices(remaining_idx, k=int(len(cc_idx)/3))
...
def run(param):
    ...
    for i in range(param.n_experts):
        ...
        else: # if only one expert, the current sets are the entire sets
            tr_set = e_tr_set
            val_set = e_val_set
            test_set = e_test_set
        ...
        # create the model and change the classifier, with the rigth number of classes
        model = resnet18()
        fc_in_features = model.fc.in_features

        # if more than one expert, we perform (1 vs all)-classification
        # we want to classify the current classes, plus the 'other' class
        if param.n_experts > 1: fc_out_features = len(current_classes)+1
        # if only one expert, standard classification
        else: fc_out_features = len(current_classes)

        model.fc = nn.Linear(fc_in_features, fc_out_features)

        # resnet18 from CGQA has 5x5 kernels
        # change it to be compatible and to load the model later, for continual training
        if param.n_experts == 1:
            model.conv1 = nn.Conv2d(3, 64, kernel_size=5, stride=2, padding=1, bias=False)

        # training process
        # inside, the labels are preprocessed for (1 vs all)-classification (if needed)
        save_path = param.save_root + f'greatest_expert/{run.name}'
        ...
        num_classes = len(current_classes)+1 if param.n_experts > 1 else len(current_classes)
        results = trainer.train(tr_loader, val_loader, num_classes=num_classes, epochs=param.max_epochs)
